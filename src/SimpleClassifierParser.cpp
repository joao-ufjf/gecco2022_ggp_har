...
SimpleClassifierParser::SimpleClassifierParser() {
    ...
    raco = new ContinuousAntColonyOptmization();
}
...
void SimpleClassifierParser::Optimize(Subject* s){
    if(conf->approach == 100 || conf->approach == 101){
        raco->Optimize(s, this);
//        s->print();
//        cin.get();
    }
    else if(conf->approach == 1 || conf->approach == 2 || conf->approach == 9 || conf->approach == 13 || conf->approach == 14 || conf->approach == 21){
