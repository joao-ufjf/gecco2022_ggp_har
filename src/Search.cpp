...
//    cout << pop[0]->fitness << " " << parserTest->Evaluate(pop[0]) << endl;
//    cin.get();
...
//     Para todos os casos que o otimizador executara no fim
    if(conf->approach == 1 || conf->approach == 6 || conf->approach == 7 || conf->approach == 8 ||
       conf->approach == 13 || conf->approach == 18 || conf->approach == 19 || conf->approach == 20){
        ...
        conf->optimizationEvaluations = maxEvaluations * 0.02;
        maxEvaluations *= 0.98;
    }
    else if (conf->approach == 101 || conf->approach == 201){
        conf->optimizationEvaluations = maxEvaluations * 0.2;
        maxEvaluations *= 0.8;
    }
...
//                cin.get();
            ...
            else if(conf->approach == 200){ // ELITE
                for(int x = 0; x < conf->popSize; x++){
                    pop[x]->fitnessValidation = parserValidation->Evaluate(pop[x]);
                }
//                cout << "VALIDADO PARA OTIMIZAR:" << endl;
...
//        stable_sort(pop, pop + conf->popSize, SortPopulationFitness);
        ...
        for(int i = 0; i < 1; i++){
            cout << "it:" << it << " n:" << conf->evaluations << " fit:" << pop[i]->fitness << " size:" << pop[i]->trees[0]->linearModel.size()  << " model:";
            pop[i]->print();
//            cout << DECounter << endl;
//            cin.get();
        }
//        cin.get();
    ...
    if(conf->approach == 100 || conf->approach == 101){ // BEST
        for(int x = 0; x < conf->popSize; x++){
            pop[x]->fitnessValidation = parserValidation->Evaluate(pop[x]);
        }
//                cout << "VALIDADO PARA OTIMIZAR:" << endl;
...
//                cin.get();
    ...
    else if(conf->approach == 200 || conf->approach == 201){ // ELITE
        for(int x = 0; x < conf->popSize; x++){
            pop[x]->fitnessValidation = parserValidation->Evaluate(pop[x]);
        }
//                cout << "VALIDADO PARA OTIMIZAR:" << endl;
...
//                cin.get();
    ...
    //Avalia população
    EvaluatePopulation(conf->popSize, conf->popSize);
    stable_sort(pop + conf->popSize, pop + conf->popSize, SortPopulationFitness);

//    cout << DECounter << endl;
//    cin.get();

    cout << "FIM DO TREINO" << endl;
    cout << "best => fit:" << pop[0]->fitness << " size:" << pop[0]->trees[0]->linearModel.size()  << " model:";
    pop[0]->print();

        /*** Com a populacao treinada, hora de realizar a validação ***/
    for(int i = 0; i < conf->popSize; i++){
        pop[i]->fitnessTest = parserTest->Evaluate(pop[i]);
    }
    stable_sort(pop, pop + conf->popSize, SortPopulationFitnessTest);

    cout << "FIM DA VALIDAÇÃO" << endl;
    cout << "best => fit:" << pop[0]->fitnessTest << " size:" << pop[0]->trees[0]->linearModel.size()  << " model:";
    pop[0]->print();

    /*** Com a populacao treinada, hora de realizar o teste ***/
    for(int i = 0; i < conf->popSize; i++){
    ...
    cout << "best => fit:" << pop[0]->fitnessValidation << " size:" << pop[0]->trees[0]->linearModel.size()  << " model:";
    pop[0]->print();

    std::ofstream final_pop_file;
    final_pop_file.open(conf->output, std::ios_base::app);
    std::streambuf *coutbuf = std::cout.rdbuf(); //save old buf
    std::cout.rdbuf(final_pop_file.rdbuf()); //redirect std::cout to out.txt!
//    cout << "fitness,model" << endl;
//    for(int i = 0; i < conf->popSize; i++){
    for(int i = 0; i < 1; i++){
        cout << conf->seed << ";" << pop[i]->fitness << ";" << pop[i]->fitnessTest << ";" << pop[i]->fitnessValidation << ";";
        pop[i]->print();
//        cin.get();
    }
...
//            cout << "Antes do otimizador" << endl;
            parser->Optimize(ind_);
//            cout << "Depois do otimizador" << endl;
